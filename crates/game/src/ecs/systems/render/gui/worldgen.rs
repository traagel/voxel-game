...
use crate::ecs::resources::window_manager::WorldGenWindowStateRes;

/// Draw the world generation configuration window
pub fn draw_worldgen_window(state: &mut WorldGenWindowStateRes) {
    ...
    root_ui().window(hash!("worldgen"), win_pos, win_size, |ui| {
        ...
        // num_craters (usize)
        let mut num_craters = state.params.num_craters as f32;
        ui.slider(hash!("num_craters"), "Num Craters", 0.0..20.0, &mut num_craters);
        state.params.num_craters = num_craters.clamp(0.0, 20.0) as usize;
        // ridge_weight (f64)
        let mut ridge_weight = state.params.ridge_weight as f32;
        ui.slider(hash!("ridge_weight"), "Ridge Weight", 0.0..1.0, &mut ridge_weight);
        state.params.ridge_weight = ridge_weight as f64;
